...
.PHONY: run
run:
	docker run --restart=always -v $(SERVER_SRC):$(DST) --env-file .env.local --env-file .env.production --name $(SERVICE_NAME) -p 3000:3000 -d $(SERVICE_NAME):latest --restart=always
	docker start $(SERVICE_NAME)
